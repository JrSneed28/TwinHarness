...
exports.runHookStopGateFromRoot = runHookStopGateFromRoot;
exports.runHookSubagentStopFromRoot = runHookSubagentStopFromRoot;
exports.runHookPretoolGateFromRoot = runHookPretoolGateFromRoot;
...
function evaluateStopGate(paths) {
    ...
    if (verdict.ok)
        return { block: false, reasons: [] };
    return { block: true, reasons: [(0, next_1.renderStopReason)(verdict.error ?? "blocked", verdict.detail)] };
}
/**
 ...
 */
function resolveOrConflict(root) {
    try {
        return { paths: (0, paths_1.resolveProjectPaths)(root) };
    }
    catch (e) {
        if (e instanceof paths_1.StateLocationConflictError)
            return { conflict: e };
 ...
 */
function runHookStopGateFromRoot(root, payload) {
    const r = resolveOrConflict(root);
    if ("conflict" in r) {
        return {
            stdout: JSON.stringify({
                decision: "block",
                reason: "TwinHarness stop-gate blocked completion: the state LOCATION is ambiguous/unsafe — " +
                    r.conflict.message,
            }),
            exitCode: 0,
        };
 ...
 */
function runHookSubagentStopFromRoot(root, payload) {
    const r = resolveOrConflict(root);
    if ("conflict" in r) {
        return {
            stdout: JSON.stringify({
                decision: "block",
                reason: "TwinHarness subagent-stop gate blocked: the state LOCATION is ambiguous/unsafe — " +
                    r.conflict.message,
            }),
            exitCode: 0,
        };
 ...
 */
function runHookPretoolGateFromRoot(root, payload, env = process.env) {
    const r = resolveOrConflict(root);
    if ("conflict" in r) {
        ...
        return fireGateResult("deny", "TwinHarness write-gate (F5 — fail-closed) DENIED this write: the state LOCATION is " +
...
function runHookSubagentStop(paths, input) {
    ...
    if (stoppingId)
        (0, delegation_scope_1.clearDelegationScope)(paths, stoppingId);
    const r = (0, state_store_1.readState)(paths);
 ...
 */
function isEvidenceAnchorPath(relFwd, paths) {
    const stateRel = toRootRelative(paths.stateDir, paths.root);
    if (stateRel === null)
        return false;
    return (relFwd === `${stateRel}/verify.json` ||
...
function runHookPretoolGate(paths, input, env = process.env) {
    ...
    const stdinScope = Array.isArray(input?.allowed_files)
        ...
        : [];
    const { active: activeScopes } = (0, delegation_scope_1.readActiveDelegationScopes)(paths);
    ...
    if (bashCommand) {
        ...
        for (const token of extractBashWriteTargets(bashCommand)) {
            ...
            if (relC1 !== null && isEvidenceAnchorPath(relC1, paths)) {
                const reason = `TwinHarness write-gate (R-19/R-31) hard-blocked a Bash-mediated write to a completion-evidence anchor (${relC1}). ` +
    ...
    if (isEvidenceAnchorPath(relFwd, paths)) {
        const reason = `TwinHarness write-gate gated a direct write to a completion-evidence anchor (${relFwd}). ` +
