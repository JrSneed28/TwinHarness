...
exports.runRepoSearch = runRepoSearch;
 ...
 */
const SEARCH_KINDS = ["literal", "regex", "symbol", "req", "artifact", "template"];
/**
...
const MAX_SCANNED_FILES = scanner_1.FILE_COUNT_CAP; // never open more files than the map's scan cap
/**
...
function runRepoSearch(paths, opts = {}) {
    ...
    if (pattern === undefined || pattern.length === 0) {
        (0, log_1.structuredLog)({ cmd: "repo search", error: "no_pattern" });
        return (0, output_1.failure)({ human: "Provide a search pattern: --pattern <p>.", data: { error: "no_pattern" } });
    }
    // ---- validate kind ----
    const kind = (opts.kind ?? "literal");
    if (!SEARCH_KINDS.includes(kind)) {
        (0, log_1.structuredLog)({ cmd: "repo search", error: "bad_kind", kind: opts.kind });
        return (0, output_1.failure)({
    ...
    if (kind === "regex") {
        try {
            ...
            (0, log_1.structuredLog)({ cmd: "repo search", error: "bad_regex" });
            return (0, output_1.failure)({
    ...
    const receiptFor = (rel, content) => {
        if (!receiptByFile.has(rel))
            receiptByFile.set(rel, { file: rel, hash: (0, hash_1.hashContent)(content) });
    };
    ...
    const readInScope = (rel) => {
        const abs = (0, paths_1.resolveWithinRoot)(paths.root, rel);
        if (abs === null)
            return null; // defense-in-depth: map paths are in-scope by construction.
        try {
            if (fs.statSync(abs).size > MAX_SEARCH_FILE_BYTES)
                return null; // name-only in the map too.
    ...
    if (kind === "literal" || kind === "regex") {
        ...
        for (const fe of map.files) {
            ...
            if (content === null)
                continue;
            filesScanned++;
            if (!scanLines(fe.path, content, matcher))
                break;
        }
    }
    else if (kind === "symbol") {
        ...
        outerSym: for (const fe of map.files) {
            if (!fe.symbols || fe.symbols.length === 0)
                continue;
        ...
        for (const a of approved) {
            ...
            if (!a.file.toLowerCase().includes(needle))
                continue;
        ...
        for (const t of names) {
            ...
            try {
                ...
                filesScanned++;
                if (!receiptByFile.has(t.rel))
                    receiptByFile.set(t.rel, { file: t.rel, hash: (0, hash_1.hashContent)(content) });
            }
            catch {
    ...
    const human = citations.length === 0
    ...
    for (const { dir, source, relPrefix } of dirs) {
        ...
        for (const e of entries) {
            if (!e.isFile())
                continue;
            if (seen.has(e.name))
                continue; // override wins (project dir scanned first).
            seen.add(e.name);
