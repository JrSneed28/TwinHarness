...
exports.runTemplateGet = runTemplateGet;
exports.runTemplateList = runTemplateList;
...
function normalizeTemplateName(name) {
    ...
    if ((0, paths_1.isAbsoluteOrEscaping)(trimmed))
        return null;
 ...
 */
function resolve(name, projectRoot, plugin) {
    ...
    for (const [abs, source] of [
        ...
        try {
            ...
            if (st.isFile())
                return { path: abs, content: fs.readFileSync(abs, "utf8"), source };
        }
        catch {
...
function runTemplateGet(paths, name) {
    if (name === undefined || name.trim() === "") {
        (0, log_1.structuredLog)({ cmd: "template get", error: "missing_name" });
        return (0, output_1.failure)({ human: "usage: th template get <name>", data: { error: "missing_name" } });
    }
    const normalized = normalizeTemplateName(name);
    if (normalized === null) {
        (0, log_1.structuredLog)({ cmd: "template get", name, error: "invalid_name" });
    ...
    if ("error" in r) {
        (0, log_1.structuredLog)({ cmd: "template get", name: normalized, error: "template_not_found" });
        return (0, output_1.failure)({
    ...
    (0, log_1.structuredLog)({ cmd: "template get", name: normalized, source: r.source });
...
function runTemplateList(paths) {
    ...
    for (const name of pluginNames)
        byName.set(name, { name, source: "plugin-bundled" });
    ...
    (0, log_1.structuredLog)({ cmd: "template list", count: templates.length, overrides: projectNames.size, shadowed: shadowed.length });
    const human = templates.length === 0
        ? `No templates found. Searched:\n  - ${projectDir}\n  - ${pluginDir}`
        : [
            `${templates.length} template(s) (resolve with \`th template get <name>\`):`,
            ...templates.map((t) => `  ${t.name}  [${t.source}]${t.shadowsBundled ? " (shadows bundled)" : ""}`),
        ].join("\n");
    return (0, output_1.success)({ data: { templates, count: templates.length }, human });
}
