...
const grounding_1 = require("./commands/grounding");
...
function dispatch(parsed) {
    ...
    if (parsed.positionals[0] === "state" && parsed.positionals[1] === "adopt") {
        ...
        if (!target) {
            return (0, output_1.failure)({
                human: "usage: th state adopt --twinharness | --legacy\n" +
                    "Consolidate two TwinHarness state locations onto one (retires the other's state.json to a backup).",
            });
        }
        const candidates = (0, paths_1.resolveStateCandidates)(parsed.flags.cwd);
        return (0, state_1.runStateAdopt)(candidates, target);
    }
    ...
    switch (group) {
        ...
        case "realize":
            return (0, realize_1.runRealize)(paths, {
                reqId: sub,
        ...
        case "grounding":
            switch (sub) {
                case "record":
                    return (0, grounding_1.runGroundingRecord)(paths, {
                ...
                case "check":
                    return (0, grounding_1.runGroundingCheck)(paths, {});
                default:
                    return (0, output_1.failure)({ human: `unknown 'grounding' subcommand: ${sub ?? "(none)"}\n\n${HELP}` });
            }
        ...
        case "context-pages":
            switch (sub) {
                ...
                case "usage":
                    return (0, context_pages_1.runContextPagesCommand)("usage", {}, paths);
