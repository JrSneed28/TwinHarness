...
function resolveExtendsTarget(fromConfigDir, ref) {
    ...
    const normPosix = (p) => {
        ...
        if (j.startsWith("..") || j === ".." || path.posix.isAbsolute(j))
            return null;
        return j;
    };
    ...
    if (ref.startsWith("/") || /^[A-Za-z]:[\\/]/.test(ref))
        return null; // absolute — never guess
...
function resolveExtendsChain(configPath, parsed, readFile) {
    ...
    const visit = (cfgPath, cfgParsed, depth) => {
        if (depth > MAX_EXTENDS_DEPTH)
            return; // depth bound — fail-safe stop
        if (seen.has(cfgPath))
            return; // cycle / dedupe bound — fail-safe stop
        seen.add(cfgPath);
