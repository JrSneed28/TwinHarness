...
const savings_classify_1 = require("./savings-classify");
// ---------------------------------------------------------------------------
...
function buildCycles(records) {
    ...
    for (const rec of records) {
        ...
        const cycle = pageId === undefined ? { base: [], rehydration: [] } : (() => {
        ...
        if (isRehydration(rec))
            cycle.rehydration.push(rec);
        else
            cycle.base.push(rec);
        if (pageId === undefined)
            singletons.push(cycle);
    }
...
function computeSavings(records, opts) {
    ...
    const scoped = sessionId !== undefined ? records.filter((r) => r.session_id === sessionId) : records.slice();
    ...
    for (const c of savings_classify_1.TELEMETRY_WORKLOAD_CATEGORIES)
        buckets.set(c, 0);
    let uncategorized = 0;
    ...
    for (const cycle of cycles) {
        ...
        for (const rec of cycle.base) {
            ...
            const cat = (0, savings_classify_1.resolveCategory)(rec);
            if (cat === undefined)
                uncategorized += contrib;
            else
                buckets.set(cat, (buckets.get(cat) ?? 0) + contrib);
        }
    ...
    const categories = savings_classify_1.TELEMETRY_WORKLOAD_CATEGORIES.map((category) => ({
        category,
