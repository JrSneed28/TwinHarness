...
exports.buildCaseStudy = buildCaseStudy;
exports.assertDogfood = assertDogfood;
exports.dogfoodCard = dogfoodCard;
...
function coverageFromBreakdown(root) {
    let result;
    try {
        result = (0, coverage_1.computeBreakdown)(root);
    }
    catch {
        return null;
    }
...
function tokenCostFromTelemetry(telemetry) {
    ...
    for (const rec of telemetry) {
        if (typeof rec.tokens === "number")
            tokens = (tokens ?? 0) + rec.tokens;
        if (typeof rec.cost === "number")
            cost = (cost ?? 0) + rec.cost;
    }
    return { tokens, cost };
}
...
function acceptanceCriteriaMet(a, coverage) {
    if (!reachedWorkingCode(a))
        return false;
    if (coverage && coverage.total > 0)
        return coverage.tested > 0;
    return true;
}
...
function dogfoodCard(a, brief) {
    ...
    return (0, assert_1.buildReportCard)("dogfood", assertions, stats);
}
