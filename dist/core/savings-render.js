 ...
 */
function renderStatusLine(result, width, color) {
    ...
    const fields = [saved, `${fmtTokens(result.avoided_tokens)} avoided`];
    if (!result.payback_measured)
        fields.push("upper bound");
...
function renderDetail(result, costLabel) {
    ...
    lines.push(`  payback:    ${result.payback_measured ? `${result.payback_tokens} tok [measured]` : "[unavailable]"}`);
