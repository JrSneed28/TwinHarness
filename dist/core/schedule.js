...
exports.scheduleWaves = scheduleWaves;
exports.conflictPairs = conflictPairs;
/**
...
function scheduleWaves(slices) {
    ...
    const place = (slice, ignoreUnmet) => {
        ...
        for (const dep of slice.depends_on ?? []) {
            if (!known.has(dep) || dep === slice.id)
                continue; // dangling/self → ignore
            const depWave = waveOf.get(dep);
            if (depWave === undefined) {
                if (!ignoreUnmet)
                    return false; // dep not placed yet — defer this slice
                continue; // forced pass: treat the unmet dep as if absent
            }
            if (depWave + 1 > minWave)
                minWave = depWave + 1;
        }
