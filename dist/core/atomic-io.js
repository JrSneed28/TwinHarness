 ...
 */
class StateWriteContendedError extends Error {
    code = "state_write_contended";
    constructor(absPath, attempts) {
        super(`could not atomically write ${absPath} after ${attempts} attempts: the file is contended ` +
    ...
    for (let attempt = 1;; attempt++) {
        try {
            ...
            if (!transient || attempt >= MAX_IO_ATTEMPTS) {
                ...
                if (transient)
                    throw new StateWriteContendedError(absPath, attempt);
    ...
    for (let attempt = 1;; attempt++) {
        try {
            ...
            if (!isTransientIoError(code) || attempt >= MAX_IO_ATTEMPTS)
                throw e;
            (0, sleep_1.sleepSync)(Math.min(4 * attempt, 40)); // zero-CPU backoff (PERF-007)
        }
    }
}
