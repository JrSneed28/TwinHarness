...
**Local data & privacy.** ContextPages' on-by-default PostToolUse hook runs in OBSERVE mode and is **metadata-only by default**: it never alters tool output and does **not** copy raw tool output (file reads, bash/web/MCP results) to disk. What it records under `.twinharness/context-pages/` (gitignored; never sent off-machine) is metadata — content **hashes**, identifiers, counts, and categories in the append-only ledger and telemetry. Raw output is persisted to the local cold store only when you **opt in** via `TH_CONTEXT_RAW_STORE=1` (or `TH_EXACT_SUPPRESS=1`, which needs the raw bytes to rehydrate). Even then, sensitive content is excluded by a best-effort secret classifier (path denylist + regex, fail-toward-sensitive) — only its hash is recorded — and raw objects are subject to size/age retention (defaults 256 MiB / 14 days; inspect with `th context-pages usage`). The ledger and telemetry are currently append-only. Set `TH_DISABLE_CONTEXT_PAGES=1` to record nothing, and clear the store with `th context-pages gc [--age-days <n>]` (cold objects only) or `th context-pages purge` (everything). Details in [docs/guide/advanced.md](./docs/guide/advanced.md#context-pages-what-lands-on-disk).
