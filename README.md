...
- **MCP sub-lease parity.** `th_build_sub_claim` and `th_build_sub_release` are now registered MCP tools, giving agents the same sub-lease capability as CLI Builders (REQ-101). The MCP registry now holds exactly **38 tools** (REQ-408): the coordination surface plus the `th_proof_run`/`th_proof_component`/`th_proof_report` operational-proof trio.
...
- `th` CLI with 1100+ tests covering CLI behavior, plugin-packaging integrity, security containment (path traversal, proto-pollution), the deterministic repo-understanding layer (scanner, schema, relevance, impact, MCP tools), decision governance (hash-chain tamper detection, TTY barrier, gating predicate), repo staleness detection, brownfield tiering gate, `th next` decision-obligation rung, and the full 38-tool MCP surface; CI runs typecheck, build, a dist-sync assertion, and the full suite on every push and PR.
