...
- **MCP sub-lease parity.** `th_build_sub_claim` and `th_build_sub_release` are now registered MCP tools, giving agents the same sub-lease capability as CLI Builders (REQ-101). The MCP registry now holds exactly **23 tools** (REQ-408).
...
**MCP tools (registered count 9 → 16 via the repo-understanding layer; then 16 → 23 via the self-epic governance layer):** `th_repo_map`, `th_repo_relevant`, `th_repo_impact`, `th_context_pack` — thin adapters over the same handlers as the CLI commands; strict closed schemas (`additionalProperties: false`). The self-epic governance work adds 7 more tools bringing the total to exactly **23**.
...
- Full T0–T3 pipeline, all 12 agents, all stages.
...
agents/           12 agent prompt files (lean cores; detail lives in skills/twinharness/reference/)
