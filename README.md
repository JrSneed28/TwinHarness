...
- **MCP sub-lease parity.** `th_build_sub_claim` and `th_build_sub_release` are now registered MCP tools, giving agents the same sub-lease capability as CLI Builders (REQ-101). The MCP registry now holds exactly **60 tools** (REQ-408): the coordination surface plus the `th_interview_start`/`th_interview_record`/`th_interview_status`/`th_init` interview/init tools plus the MCP-tool-expansion's 5 typed gate-transition tools (`th_tier_record`/`th_stage_advance`/`th_implementation_unlock`/`th_write_gate_set`/`th_blast_radius_record`) and 16 wired handlers (drift/coverage/artifact/verify/stage/doctor/scorecard/slice families).
...
**MCP tools (registered count 9 → 16 via the repo-understanding layer; then 16 → 35 via the self-epic governance and coordination layers; then 35 → 39 via the interview/init layer; then 39 → 60 via the MCP-tool-expansion):** `th_repo_map`, `th_repo_relevant`, `th_repo_impact`, `th_context_pack` — thin adapters over the same handlers as the CLI commands; strict closed schemas (`additionalProperties: false`). The self-epic governance and coordination work added 19 more tools (to 35), the interview/init layer appends `th_interview_start`/`th_interview_record`/`th_interview_status`/`th_init` reaching 39, and the MCP-tool-expansion adds 5 typed gate-transition tools + 16 wired handlers for a total of exactly **60**.
...
- `th` CLI with 1100+ tests covering CLI behavior, plugin-packaging integrity, security containment (path traversal, proto-pollution), the deterministic repo-understanding layer (scanner, schema, relevance, impact, MCP tools), decision governance (hash-chain tamper detection, TTY barrier, gating predicate), repo staleness detection, brownfield tiering gate, `th next` decision-obligation rung, and the full 60-tool MCP surface; CI runs typecheck, build, a dist-sync assertion, and the full suite on every push and PR.
