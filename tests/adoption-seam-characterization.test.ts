...
describe("SLICE-0 characterization: MCP tool registry baseline (Seam B)", () => {
   ...
   * Anchor: REQ-105
   */
  it("REQ-105: test_REQ105_tool_count_incremental_path_16_to_42 — TOOL_DEFS.length is exactly 42 after the coordination-primitive layer (th_build_dispatch/plan + th_artifact_*/collab_*/debate_*) plus the proof suite (th_proof_run/component/report) plus the interview/init tools (th_interview_*/th_init)", () => {
    ...
    // interview/init tools (th_interview_*/th_init) append → 42.
...
describe("SLICE-0 characterization: th_decision_approve absent from TOOL_DEFS (Seam B)", () => {
   ...
   * Anchor: REQ-408
   */
  it("REQ-408: test_REQ408_mcp_has_no_decision_approve_tool_count_42 — th_decision_approve absent from TOOL_DEFS; count invariant holds at 42", () => {
    ...
    // th_proof_run/component/report → 38; th_interview_*/th_init → 42. th_decision_approve
    // stays permanently absent (decision approval is a human gate, never an MCP tool).
    expect(TOOL_DEFS.length).toBe(42);
  });
});
