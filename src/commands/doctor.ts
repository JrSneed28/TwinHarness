...
export function runDoctor(paths: ProjectPaths, opts: { strict?: boolean } = {}): CommandResult {
  ...
  try {
    ...
    checks.push({
      ...
      status: warn ? "warn" : "ok",
      detail:
        `total ${fmtBytes(s.total_bytes)} / ${fmtBytes(s.aggregate_max_bytes)} aggregate cap (${s.aggregate_pct}%); ` +
        `cold ${s.cold_objects} obj ${fmtBytes(s.cold_bytes)} / ${fmtBytes(s.max_bytes)} (${coldPct}%); ` +
        `ledger ${fmtBytes(s.ledger_bytes)} + telemetry ${fmtBytes(s.telemetry_bytes)} (append-only)` +
