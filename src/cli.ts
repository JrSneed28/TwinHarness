...
import { runVerifyRun, runVerifyAdd, runVerifyList, runVerifyClear, runVerifyApprove } from "./commands/verify";
...
import { writeDelegationScope } from "./core/delegation-scope";
 ...
 * is needed here.
 */
function buildScanOptions(flags: ParsedArgs["flags"]): { fileCountCap?: number; totalBytesCap?: number } {
  const out: { fileCountCap?: number; totalBytesCap?: number } = {};
  if (typeof flags.maxFiles === "number") out.fileCountCap = flags.maxFiles;
  if (typeof flags.maxBytes === "number") out.totalBytesCap = flags.maxBytes;
  return out;
}
    ...
    if (name in BOOLEAN_FLAGS) {
      ...
      if (name === "--read-only") {
        process.stderr.write(
          "warning: --read-only is deprecated; use --no-obvious-writes\n",
        );
      }
...
function dispatch(parsed: ParsedArgs): CommandResult {
  ...
  if (parsed.positionals[0] === "state" && parsed.positionals[1] === "adopt") {
    ...
    if (parsed.flags.twinharness && parsed.flags.legacy) {
      return failure({
        human:
          "Pass only one of --twinharness / --legacy.\n" +
  ...
  switch (group) {
    ...
    case "delegate":
      switch (sub) {
        ...
        case "pack": {
          const packRes = runDelegatePack(paths, {
            agent: parsed.flags.agent,
            task: parsed.flags.task,
            intent: parsed.flags.intent,
            slice: parsed.flags.slice,
    ...
    case "research":
      switch (sub) {
        case "write":
          return runResearchWrite(paths, { topic: parsed.flags.topic, markdown: parsed.flags.markdown, version: parsed.flags.version });
        default:
    ...
    case "verify":
      switch (sub) {
        case "run":
          ...
          return runVerifyRun(paths, { readOnly: parsed.flags.readOnly });
        case "add":
          // P6-2 (#19): --as records the actor in per-command provenance.
          return runVerifyAdd(paths, rest.join(" "), { as: parsed.flags.as });
        case "approve":
          // P6-2 (#19): human-confirm the current command set for execution.
          return runVerifyApprove(paths, { as: parsed.flags.as });
        case "list":
...
function mapDispatchError(e: unknown): CommandResult {
  ...
  if (code === "schema_too_new") {
    const err = e as { onDisk?: number; current?: number; message: string };
    return failure({
      human: err.message,
