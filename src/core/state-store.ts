 ...
 * is the problem.
 */
export class LockStampError extends Error {
  readonly code = "state_lock_stamp_failed";
  constructor(lockDir: string) {
    super(
      `state lock owner-stamp failed: could not write the owner token under ${lockDir} ` +
...
export class SchemaTooNewError extends Error {
  ...
  constructor(
    ...
    public readonly onDisk: number | undefined,
 ...
 */
export function assertWriteAllowed(paths: ProjectPaths): void {
  ...
  try {
    ...
    throw new SchemaTooNewError(
      undefined,
      CURRENT_SCHEMA_VERSION,
      `Refusing to mutate state.json: the file is present but could not be read to verify its schema version. ` +
  ...
  try {
    ...
    throw new SchemaTooNewError(
      undefined,
      CURRENT_SCHEMA_VERSION,
      `Refusing to mutate state.json: the on-disk file is present but not valid JSON (corrupt or partially written). ` +
  ...
  if (!result.ok) {
    ...
    throw new SchemaTooNewError(
      undefined,
      CURRENT_SCHEMA_VERSION,
      `Refusing to mutate state.json: the on-disk file is present but does not validate (corrupt or partially written). ` +
  ...
  if (onDisk === undefined || onDisk <= CURRENT_SCHEMA_VERSION) return;
  ...
  throw new SchemaTooNewError(
    onDisk,
    CURRENT_SCHEMA_VERSION,
    `Refusing to mutate state.json: it is schema v${onDisk}, newer than this th (v${CURRENT_SCHEMA_VERSION}). ` +
...
export const DEFAULT_LOCK_TIMEOUT_MS = 25_000;
...
export function lockTimeoutMs(): number {
  ...
  if (raw === undefined) return DEFAULT_LOCK_TIMEOUT_MS;
  const n = Number(raw);
