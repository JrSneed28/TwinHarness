...
import { PROOF_COMPONENT_NUMBERS, type CoverageDimension, type ProofReport } from "./types";
...
export function defaultOutputRoot(repoRoot: string): string {
  const dir = resolveWithinRoot(repoRoot, path.join(".twinharness", "proof"));
  if (dir === null) {
...
function timestampDir(iso: string): string {
 ...
 */
export function toJsonl(report: ProofReport): string {
  const lines: string[] = [];
...
function dimensionRow(label: string, dim: CoverageDimension): string {
...
/** The on-disk artifact paths {@link emitReport} wrote. */
 ...
 */
export function emitReport(report: ProofReport, opts: { outputRoot: string }): EmittedReport {
