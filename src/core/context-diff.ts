...
function preferCrlf(s: string): boolean {
  const crlf = (s.match(/\r\n/g) ?? []).length;
  const lf = (s.match(/\n/g) ?? []).length;
...
export function reconstruct(baseContent: string, patch: DeltaPatch): string {
  try {
    ...
    for (const eol of eolCandidates) {
      ...
      if (!patch.current_hash || hashContent(candidate) === patch.current_hash) {
        return candidate;
      }
    }
...
export function computeDelta(
  ...
  try {
    ...
    if (baseLines.length + currentLines.length > MAX_DIFF_LINES) {
