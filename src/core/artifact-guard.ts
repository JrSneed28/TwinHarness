...
import type { ApprovedArtifact } from "./state-schema";
import { resolveWithinRoot } from "./paths";

/** Stable machine token surfaced when a registered-artifact overwrite is refused. */
export const APPROVED_ARTIFACT_CLOBBER_CODE = "approved_artifact_clobber";
 ...
 */
function toArtifactKey(root: string, target: string): string | null {
 ...
 */
export function matchApprovedArtifact(
  ...
  for (const a of approved) {
    if (a.file === key) return a; // exact file-artifact match
 ...
 */
export function isApprovedArtifactPath(
  approved: readonly ApprovedArtifact[],
  root: string,
  target: string,
): boolean {
