...
import { SEED_DIMENSION_NAMES } from "./verification-driver";
 ...
 * runtime self-attest.
 */
export const DECLARED_DIMENSION_SET: readonly string[] = ["tests-executed", "typecheck", "build"];
 ...
 * set's identity is.
 */
export function declaredDimensionSetCanonical(): string {
 ...
 */
export function declaredDimensionSetDigest(): string {
 ...
 */
export function declaredDimensionSet(): string[] {
 ...
 */
export function assertDeclaredDimensionsObservable(): void {
  ...
  if (unobservable.length > 0) {
    throw new Error(
      `DECLARED_DIMENSION_SET contains dimension(s) the BSC-3 sensor cannot observe (not in SEED_DIMENSION_NAMES): ${unobservable.join(", ")}. ` +
