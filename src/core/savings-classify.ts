...
export const TELEMETRY_WORKLOAD_CATEGORIES: readonly TelemetryWorkloadCategory[] = [
  "file-read",
  "artifact-summary",
  "repo-analysis",
  "test-output",
...
const CATEGORY_SET = new Set<string>(TELEMETRY_WORKLOAD_CATEGORIES);

/** Inputs to {@link classify}. All optional — partial inputs are tolerated. */
export interface ClassifyInput {
  /** Dispatcher operation, if this record is for one (e.g. "rehydrate"). */
  op?: string;
  /** True when this record represents a full rehydration / re-serve. */
  full_rehydration?: boolean;
  /** True when this record represents a compaction / epoch reset. */
  compaction?: boolean;
  /** Tool label (e.g. "Read", "Grep", "Glob", "Bash", "mcp__github__…"). */
  tool_type?: string;
  /** Source kind of the underlying page. */
  source_kind?: SourceKind;
  /** Reduction kind (e.g. "FULL", "lossy", "hash-only"). */
  reduction_kind?: string;
  /**
   * Raw Bash command text — WRITE TIME ONLY, used for rules 5/6 then discarded.
   * NEVER persist this onto a telemetry record.
   */
  command?: string;
  /** Marker that the page is an agent/explore summary (rule 7). */
  is_summary?: boolean;
}
 ...
 */
export function classify(input: ClassifyInput): TelemetryWorkloadCategory {
  // 1. rehydration
  ...
  if (
    (input.tool_type !== undefined && FILE_READ_TOOLS.has(input.tool_type)) ||
  ...
  if (
    (input.command !== undefined && REPO_CMD.test(input.command)) ||
 ...
 */
export function resolveCategory(rec: TelemetryRecord): TelemetryWorkloadCategory | undefined {
  const wc = rec.workload_category;
  if (typeof wc === "string" && CATEGORY_SET.has(wc)) {
    return wc as TelemetryWorkloadCategory;
  }
  if (wc === "planning") {
    return "debug-output";
  }
