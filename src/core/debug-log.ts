...
export type DebugStatus = "open" | "resolved";

export interface DebugEntry {
  /** `DEBUG-NNN` */
  id: string;
  /** The parenthetical ref, e.g. `REQ-007 / SLICE-2`. */
  ref: string;
  symptom: string;
  evidence: string;
  rootCause: string;
...
export function formatDebugEntry(entry: DebugEntryInput): string {
...
export function parseDebugEntries(text: string): DebugEntry[] {
  const entries: DebugEntry[] = [];
  let current: DebugEntry | undefined;
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (current) {
      ...
      if (field) {
        const value = field[2]!;
        if (field[1] === "Symptom") current.symptom = value;
        else if (field[1] === "Evidence") current.evidence = value;
        else if (field[1] === "RootCause") current.rootCause = value;
        else current.status = (value === "resolved" ? "resolved" : "open");
      }
    }
  }
  if (current) entries.push(current);
...
export function nextDebugId(text: string): string {
  let max = 0;
