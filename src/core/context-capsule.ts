 ...
 */
const MANDATORY_ARRAY_FIELDS: ReadonlyArray<keyof Capsule> = [
  "open_blocking_drift",
  "blast_radius_flags",
  "approved_constraints",
  "requirement_ids",
...
function truncateArrayField(value: unknown, max: number): unknown {
...
function enforceBudget(body: CapsuleBody, budgetTokens: number): CapsuleBody {
  ...
  if (estimateBodyTokens(current) > HARD_CAP_TOKENS) {
    let max = 64;
    while (estimateBodyTokens(current) > HARD_CAP_TOKENS && max >= 0) {
      for (const field of MANDATORY_ARRAY_FIELDS) {
        current = {
          ...current,
          [field]: truncateArrayField(current[field as keyof CapsuleBody], max),
        };
      }
